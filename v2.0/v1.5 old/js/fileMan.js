...
export let selectedPalette = ['#7a8589', '#328e98', '#a1d1bf', '#efc063', '#cd742f']; 
export let selectedDifferencePalette = ['#727786', '#8490a3', '#9ba9bd', '#bec8d3', '#eceeec'];
...
export async function loadTerrainData(filesetKey) {
    ...
    try {
        ...
        const variableIndex = 3; // L'indice del terreno e' la quarta variabile (indice 3)
...
export async function updateFileset(filesetKey) {
    ...
    try {
        ...
        if (state[filesetKey]?.edxData?.sumSpacing) {
            resizeChartContainers();
        } else {
            console.warn(`sumSpacing non disponibile per ${filesetKey}`);
        }
...
export function populateColorPaletteSelector(paletteOptions, isDifference = false) {
    ...
    for (const category in COLOR_PALETTES) {
        for (const paletteNumber in COLOR_PALETTES[category]) {
            ...
            name.textContent = `${category} ${paletteNumber}`;
...
export async function initializeDataMenu() {
    if (!DOM.selectDataGroup || !DOM.selectData || !DOM.selectDataGroup.options.length) {
        console.error("Elementi select necessari non trovati o vuoti!");
        return;
    }
...
// Funzione per ribaltare la palette
export function reversePalette(palette) {
    if (Array.isArray(palette)) {
        return palette.slice().reverse();
    }
    console.warn('La palette non è un array:', palette);
    return palette; // Restituisci la palette originale se non è un array
}

// Aggiungi event listener per i pulsanti di ribaltamento
