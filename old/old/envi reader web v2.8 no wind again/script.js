...
setInterval(async () => {
    if (app.timeSlider && app.availableTimes.length > 0) {
        const currentTimeIndex = parseInt(app.timeSlider.value);
        await app.loadData(app.availableTimes[currentTimeIndex]);
        app.updatePlot();
    }
}, 5000);
...
const app = {
    ...
    init() {
        ...
        this.timeSlider = document.getElementById('timeSlider');  // Aggiungi questa riga
    ...
    setupEventListeners() {
        ...
        document.getElementById('timeSlider').addEventListener('change', (e) => app.updateTime(e.target.value));
        document.getElementById('variableSelect').addEventListener('change', () => this.updatePlot());
        document.getElementById('zLevelSlider').addEventListener('change', () => app.updatePlot());
        ...
        document.getElementById('timeSlider').addEventListener('input', (e) => updateTimeDisplay(e.target.value));
    ...
    async updateTime(index) {
        if (this.availableTimes.length > 0) {  // Aggiungi questo controllo
