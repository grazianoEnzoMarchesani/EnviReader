...
// Override addEventListener to force passive option for 'wheel' and 'mousewheel' events
const originalAddEventListener = EventTarget.prototype.addEventListener;
EventTarget.prototype.addEventListener = function (type, listener, options) {
    if ((type === 'wheel' || type === 'mousewheel') && options === undefined) {
...
function updatePlot(chart, currentData, terrainData, metadata, variableIndex, zLevel, followTerrain, colormap, windData) {
    ...
    const showWindFlow = document.getElementById('showWindFlow').checked;
    ...
    if (showWindFlow && windData && windData.processedData && windData.processedData.length > zLevel) {
        ...
        if (windVectors.length > 0) {
            ...
            option.visualMap.push({
             ...
             windFlowColor = document.getElementById('windFlowColor').value;
...
function setupWindFlowControls() {
    const showWindFlow = document.getElementById('showWindFlow');
    const windFlowControls = document.getElementById('windFlowControls');

    showWindFlow.addEventListener('change', function() {
        windFlowControls.style.display = this.checked ? 'block' : 'none';
        app.updatePlot();
    });
    ...
    particleDensity.addEventListener('input', () => app.updatePlot());
