...
const app = {
    ...
    setupEventListeners() {
        ...
        document.getElementById('windOpacity').addEventListener('input', () => this.updatePlot());
    },
    ...
    async loadData(selectedTime) {
        try {
            ...
            if (files.EDT && files.EDX) {
                ...
                // Load wind data
                await this.loadWindData(selectedTime);
    ...
    async loadWindData(selectedTime) {
        try {
            const atmosphereFolder = await this.mainFolder.getDirectoryHandle('atmosphere');
            this.log('Atmosphere folder found');
            const files = this.fileMapping[selectedTime];
            const windComponents = ['Flow_u', 'Flow_v', 'Flow_w'];
            this.windData = {};

            for (const component of windComponents) {
                const edtFileName = files.EDT.replace('.EDT', `_${component}.EDT`);
                const edxFileName = edtFileName.replace('.EDT', '.EDX');
                
                this.log(`Trying to load wind component: ${component}`);
                this.log(`EDT file name: ${edtFileName}`);
                this.log(`EDX file name: ${edxFileName}`);

                const edtFile = await atmosphereFolder.getFileHandle(edtFileName);
                const edxFile = await atmosphereFolder.getFileHandle(edxFileName);
                
                const edtContent = await readFileContent(edtFile);
                const edxContent = await readFileContent(edxFile);
                
                const { data } = processEDXEDT(edxContent, edtContent);
                this.windData[`${component} (m/s)`] = data[0]; // Assume there's only one variable per file
                this.log(`Wind component ${component} loaded successfully`);
            }

            this.log('Wind data loaded successfully');
        } catch (error) {
            this.log('Error loading wind data: ' + error.message, 'error');
    ...
    updatePlot() {
        ...
        try {
            ...
            const windSettings = getWindSettings();
