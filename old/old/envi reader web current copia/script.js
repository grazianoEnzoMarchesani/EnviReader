...
function updatePlot(chart, currentData, terrainData, metadata, variableIndex, zLevel, followTerrain, colormap, windData, showWindField, windDensity, windScale, windColor) {
    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: function (params) {
                if (params.seriesType === 'heatmap') {
                    ...
                    return `X: ${params.data[0]}<br>Y: ${params.data[1]}<br>Value: ${value !== null && value !== undefined ? value.toFixed(2) : 'N/A'}`;
                } else if (params.seriesType === 'custom') {
                    const [x, y, u, v, magnitude] = params.data;
                    return `X: ${x}<br>Y: ${y}<br>U: ${u !== undefined ? u.toFixed(2) : 'N/A'}<br>V: ${v !== undefined ? v.toFixed(2) : 'N/A'}<br>Magnitude: ${magnitude !== undefined ? magnitude.toFixed(2) : 'N/A'}`;
                }
            }
        },
        title: {
        ...
        grid: {
            ...
            height: chart.getHeight() * aspectRatio
        },
        visualMap: [{
            min: visualMapMin,
            max: visualMapMax,
    ...
    if (showWindField && windData && windData.processedData && windData.processedData.length > zLevel) {
        let windVectors = [];
        const skip = 100 / windDensity;
        ...
        for (let y = 0; y < metadata.nr_ydata; y += skip) {
            for (let x = 0; x < metadata.nr_xdata; x += skip) {
                ...
                if (adjustedZ >= 0 && adjustedZ < windData.processedData.length) {
                    const { u, v, w, mag } = windData.processedData[adjustedZ][y][x];
                    const direction = Math.atan2(v, u) * (180 / Math.PI);
                    windVectors.push([x, metadata.nr_ydata - 1 - y, direction, mag]);
                }
            }
        }
        ...
        if (windVectors.length > 0) {
            ...
            windVectors.forEach(vector => {
                windValMin = Math.min(windValMin, vector[3]);
                windValMax = Math.max(windValMax, vector[3]);
            });
            ...
            option.series.push({
                ...
                renderItem: function (params, api) {
                    const cellWidth = option.grid.width / metadata.nr_xdata;
                    const cellHeight = option.grid.height / metadata.nr_ydata;
                    const centerX = api.coord([api.value(0), api.value(1)])[0];
                    const centerY = api.coord([api.value(0), api.value(1)])[1];

                    const intensity = api.value(3) * windScale;
                    const angle = api.value(2);
                    const radians = (angle - 90) * Math.PI / 180;

                    const tipX = centerX + intensity * Math.cos(radians);
                    const tipY = centerY + intensity * Math.sin(radians);

                    const baseWidth = intensity * 0.3;
                    const baseAngle = radians + Math.PI / 2;
                    const baseLeftX = centerX + baseWidth * Math.cos(baseAngle);
                    const baseLeftY = centerY + baseWidth * Math.sin(baseAngle);
                    const baseRightX = centerX - baseWidth * Math.cos(baseAngle);
                    const baseRightY = centerY - baseWidth * Math.sin(baseAngle);
...
function updateZLevelSlider(metadata) {
    ...
    if (metadata && metadata.nr_zdata) {
        select.innerHTML = ''; // Clear existing options
        for (let i = 0; i < metadata.nr_zdata; i++) {
            const option = document.createElement('option');
            option.value = i;
...
const app = {
    ...
    setupEventListeners() {
        ...
        document.getElementById('timeSelect').addEventListener('change', (e) => this.updateTime(e.target.value));
        document.getElementById('variableSelect').addEventListener('change', () => this.updatePlot());
        document.getElementById('zLevelSelect').addEventListener('change', () => this.updatePlot());
        ...
        // New event listeners for wind field controls
        document.getElementById('showWindField').addEventListener('change', () => this.updatePlot());
        document.getElementById('windDensity').addEventListener('input', () => this.updatePlot());
        document.getElementById('windScale').addEventListener('input', () => this.updatePlot());
        document.getElementById('windColor').addEventListener('input', () => this.updatePlot());
    }
    ,
            ...
            if (files.EDT && files.EDX) {
                ...
                try {
                    ...
                    if (this.windData) {
                        ...
                        this.logWindData(parseInt(document.getElementById('zLevelSelect').value));
                    } else {
    ...
    updatePlot() {
        ...
        try {
            ...
            const zLevel = parseInt(document.getElementById('zLevelSelect').value);
            ...
            const showWindField = document.getElementById('showWindField').checked;
            const windDensity = parseInt(document.getElementById('windDensity').value);
            const windScale = parseInt(document.getElementById('windScale').value);
            const windColor = document.getElementById('windColor').value;
    
            this.log(`Updating plot. Variable: ${variableIndex}, Z-level: ${zLevel}, Follow Terrain: ${followTerrain}, Show Wind Field: ${showWindField}, Wind Density: ${windDensity}, Wind Scale: ${windScale}, Wind Color: ${windColor}`);
    
            updatePlot(this.chart, this.currentData, this.terrainData, this.metadata, variableIndex, zLevel, followTerrain, colormap, this.windData, showWindField, windDensity, windScale, windColor);
            if (this.windData) {
    ...
    logWindData(zLevel) {
        if (this.windData && this.windData.processedData && this.windData.processedData[zLevel]) {
            const zData = this.windData.processedData[zLevel];
            if (!zData) {
                this.log('No wind data available for this Z-level', 'warn');
                return;
            }
