...
function setupWindFlowControls() {
    ...
    const windFlowColor = document.getElementById('windFlowColor');

    particleDensity.addEventListener('input', () => {
        const densityValue = parseInt(particleDensity.value);
        toggleWindFlow(densityValue > 0);
        app.updatePlot();
    });
    particleSize.addEventListener('input', () => app.updatePlot());
    particleOpacity.addEventListener('input', () => app.updatePlot());
    windFlowColor.addEventListener('input', () => app.updatePlot());
}

function toggleWindFlow(show) {
    const windFlowControls = document.getElementById('windFlowControls');
    windFlowControls.style.display = show ? 'block' : 'none';
}

// Aggiungi questa chiamata alla funzione init() dell'app
setupWindFlowControls();
...
const app = {
    ...
    async init() {
        ...
        setupWindFlowControls(); // Chiamata per inizializzare i controlli del flusso del vento
    },
    ...
    setupEventListeners() {
        ...
        document.getElementById('particleDensity').addEventListener('input', () => this.updatePlot());
        document.getElementById('particleSize').addEventListener('input', () => this.updatePlot());
        document.getElementById('particleOpacity').addEventListener('input', () => this.updatePlot());
        document.getElementById('windFlowColor').addEventListener('input', () => this.updatePlot());
    },
    ...
    updatePlot() {
        ...
        try {
            ...
            const showWindFlow = particleDensity > 0;

            this.log('Updating plot. Variable: ' + variableIndex + ', Z-level: ' + zLevel + ', Follow Terrain: ' + followTerrain);

            updatePlot(this.chart, this.currentData, this.terrainData, this.metadata, variableIndex, zLevel, followTerrain, colormap, this.windData, showWindFlow);
            if (this.windData) {
...
function updatePlot(chart, currentData, terrainData, metadata, variableIndex, zLevel, followTerrain, colormap, windData, showWindFlow) {
    ...
    if (showWindFlow && windData && windData.processedData && windData.processedData.length > zLevel) {
        let windVectors = [];
        const skip = 1;  // Ridotto per avere più vettori
        ...
        if (windVectors.length > 0) {
            ...
            const particleOpacity = parseFloat(document.getElementById('particleOpacity').value);
            const windFlowColor = document.getElementById('windFlowColor').value;
            ...
            option.series.push({
                ...
                renderItem: function (params, api) {
                    const x = api.value(0),
                          ...
                          u = api.value(2),
                          v = api.value(3);
                    const start = api.coord([x - u / 2, y - v / 2]);
                    const end = api.coord([x + u / 2, y + v / 2]);
                    return {
                        type: 'line',
                        shape: {
                            x1: start[0],
        ...
        console.log('Wind flow visualization is disabled or no wind data available for plotting');
    }
